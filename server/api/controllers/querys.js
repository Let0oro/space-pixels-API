const genQuerys = (table) => (!table ? undefined : {
  ...
  get: `SELECT * FROM ${table} WHERE player_id = $1;`,
  post:
    (table == "player" &&
      `INSERT INTO player(name, email, password) VALUES ($1, $2, $3);`) ||
    (table == "ship" && `INSERT INTO ship(player_id, pixels) VALUES($1, ARRAY[$2]);`) || 
  ...
  update:
    ...
    (table == "ship" && `UPDATE ship SET pixels=(ARRAY[$1]) WHERE id=$2;`) || 
