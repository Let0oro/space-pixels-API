...
const getSessionPlayer = async (req, res) => {
  ...
  if (req.session && (req.session.playerId || req.session.playername)) {
    ...
    const {rows: [player]} = await pool.query("SELECT * FROM player WHERE id=$1 OR name=$2", [id, name]);
    return res
    .status(200)
    .json(player);
  } else {
...
const newPlayer = async (req, res) => {
  ...
  try {
    const existedPlayer = await pool.query(getExistedPlayerQuery, [name, email]);
    if (existedPlayer.rowCount)
      return res.status(400).json({
    message: "Player already exists with this name or email, try with other",
  });
...
const loginPlayer = async (req, res) => {
  try {
    ...
    const {rowCount: existedPlayer, rows: [{password: playerPassword}]} = await pool.query(getExistedPlayerQuery, [
      nameoremail,
      nameoremail,
    ]);
...
const updatePlayerSelect = async (req, res) => {
  ...
  try {
    await pool.query("UPDATE player SET active_ship_id = $1 WHERE id = $2", [n_selected, id]);
    return res.status(201).json({message: `Player ${id} selected ship updated`})
  } catch (error) {
    return res.status(400).json({message: error});
  }
}
